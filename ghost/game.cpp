...
bool CGame :: Update( void *fd, void *send_fd )
{
	...
	//CGamePlayer *Player = new CGamePlayer(m_OwnerName);
...
int votesEnd = 0;
int needVotes = GetNumHumanPlayers();
if(Command == "voteend")
{
    //Start
    if(Payload.empty( ) && m_GameLoading && m_GameLoaded) //creating a vote
    {
        if(votesEnd != 0)
        {
            SendChat(player, "Vote already initiated");
            SendAllChat("To end, we need " + UTIL_ToString(GetNumHumanPlayers()) + " votes");
            SendAllChat("VoteEnd started by " + User);
        }
        else
        {
            votesEnd = 1;
    ...
    //Voting
    if(payload =="yes") //registering a vote
    {
        SendAllChat("YES by " + User);
        votesEnd++;
    }
    //Disabled due possible bug
    //if(payload =="no") //unregistering a vote
    //
    {
        //    SendAllChat("NO by " + User);
        //    votesEnd--;
        //
    }
    //Do something
    if(votesEnd == needVotes)
    {
        //End game here
        StopPlayers("was disconnected (Game ended by voting)");
    }
}
